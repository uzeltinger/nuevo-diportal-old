...
<form action="<?php echo JRoute::_('index.php?option=com_properties&view=profiles&layout=modal&tmpl=component');?>" method="post" name="adminForm">
	...
	<table class="adminlist">
		<thead>
			<tr>
				<th class="left">
					<?php echo JHtml::_('grid.sort', 'Profile Name', 'a.name', $listDirn, $listOrder); ?>
				</th>
		...
		<?php
			...
			<tr class="row<?php echo $i % 2; ?>">
				<td>
					<a class="pointer" onclick="if (window.parent) window.parent.<?php echo $function;?>('<?php echo $item->mid; ?>', '<?php echo $this->escape(addslashes($item->name)); ?>');">
