...
class PropertiesModelStates extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				'list.select',
				'a.id, a.name, a.alias, a.parent, a.checked_out, a.checked_out_time, a.published, a.ordering')
		);
		$query->from('#__properties_state AS a');		
		// Join over the Country parent
		$query->select('cy.name AS country_name');
		$query->join('LEFT', '`#__properties_country` AS cy ON cy.id = a.parent');		
