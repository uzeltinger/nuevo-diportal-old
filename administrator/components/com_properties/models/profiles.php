...
class PropertiesModelProfiles extends JModelList
{
	public function __construct($config = array())
	{
		if (empty($config['filter_fields'])) {
			$config['filter_fields'] = array(
				...
				'name', 'a.name',
				'ordering', 'a.ordering',
				'a.canaddimages', 'a.canaddproperties',
				'propertiesadded',
			);
		}
	...
	protected function getListQuery($resolveFKs = true)
	{
		...
		$query->from('#__properties_profiles AS a');
	
		$query->select('count(p.id) as propertiesadded');
		$query->join('LEFT', '#__properties_products AS p ON p.agent_id = a.id');
